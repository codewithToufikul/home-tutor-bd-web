...
const STATS = [
  { label: 'Total Requests', value: '4', icon: History, color: 'bg-purple-500', trend: 'Active', path: '/guardian/requests' },
  { label: 'Hired Tutors', value: '2', icon: Users, color: 'bg-emerald-500', trend: 'Ongoing', path: '/guardian/requests' },
  { label: 'Saved Tutors', value: '15', icon: Heart, color: 'bg-rose-500', trend: '+3 new', path: '/guardian/saved' },
  { label: 'Messages', value: '8', icon: MessageSquare, color: 'bg-blue-500', trend: '2 unread', path: '/guardian/messages' },
];
...
export default function GuardianDashboard() {
  ...
  const [profile, setProfile] = useState({
    name: 'Home Tutor Provider BD',
    email: 'hometutorproviderbd@gmail.com',
    phone: '8801700000000',
    address: 'Mirpur 2, Dhaka',
    childClass: 'Class 10 (Science)',
    avatar: 'https://api.dicebear.com/7.x/avataaars/svg?seed=guardian',
    isUpdated: false
  });
  ...
  useEffect(() => {
    if (!profile.isUpdated) {
      const interval = setInterval(() => {
        setAlertMsg('⚠️ Please update your guardian profile, address, and child details to complete verification!');
        setShowAlert(true);
  ...
  const MY_REQUESTS = [
    { 
      id: 'REQ-402', 
      title: 'Need Class 10 Math Tutor for Son', 
  ...
  const handleProfileSave = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      setProfile({ ...profile, isUpdated: true });
      triggerToast('✅ Guardian profile, address & child details updated successfully!');
    }, 1000);
  };

  return (
    <GuardianLayout>
      <div className="max-w-6xl mx-auto space-y-8 relative pb-12">
        ...
        {/* Welcome Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/60 backdrop-blur-xl p-8 rounded-[40px] border border-white/40 shadow-xl shadow-ink/5">
          <div className="space-y-1">
            <h1 className="text-3xl font-display font-black text-ink tracking-tight">
              Guardian Dashboard
            </h1>
            <p className="text-sm font-medium text-ink-muted">
              Manage your tutor requests and find the perfect teacher for your child.
            </p>
          </div>
          <Link to="/request-tutor" className="bg-secondary text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-secondary/20 hover:bg-secondary-dark transition-all active:scale-95 flex items-center justify-center gap-2 text-center cursor-pointer">
            <PlusCircle size={18} />
            Post New Job
          </Link>
        </div>
                  ...
                  selectedApplicants.map((tutor) => (
                    <div key={tutor.id} className="p-4 bg-gray-50 rounded-2xl border border-ink/10 flex items-center justify-between gap-4">
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-secondary text-white rounded-xl font-bold text-xs uppercase hover:bg-secondary-dark transition-all cursor-pointer"
                      >
                        View Profile
                      </button>
                    </div>
          ...
          <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl space-y-5 relative text-center">
              ...
              <button 
                ...
                className="w-full py-3 bg-secondary text-white rounded-xl font-bold text-xs uppercase cursor-pointer hover:bg-secondary-dark transition-all"
              >
                Confirm Hire
              </button>
            </motion.div>
          ...
          {STATS.map((stat) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[10px] font-black text-secondary bg-secondary/5 px-2.5 py-1 rounded-lg">
                  {stat.trend}
                </span>
              </div>
              <div className="space-y-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between px-2">
              ...
              <Link to="/guardian/requests" className="text-xs font-black text-secondary hover:underline uppercase">View All</Link>
            </div>
                  ...
                  <div key={req.id} className="p-6 hover:bg-white/40 transition-colors group">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-1">
                          <h3 className="font-black text-ink group-hover:text-secondary transition-colors text-sm">{req.title}</h3>
            ...
            {/* Profile, Address & Child Details Update Section */}
            <div className="bg-white/60 backdrop-blur-xl p-8 rounded-[32px] border border-white/40 shadow-xl shadow-ink/5 space-y-6 mt-8">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-display font-black text-ink">Update Guardian Profile, Address & Child Details</h3>
                {!profile.isUpdated && (
              ...
              <form onSubmit={handleProfileSave} className="space-y-6">
                <div className="flex items-center gap-6">
                  <div className="relative">
                    ...
                    <label className="absolute -bottom-2 -right-2 w-9 h-9 bg-secondary text-white rounded-xl flex items-center justify-center shadow-lg hover:bg-secondary-dark transition-all cursor-pointer">
                      <Camera size={16} />
                  ...
                  <div>
                    <h4 className="font-black text-ink text-base">{profile.name}</h4>
                    <p className="text-xs text-ink-muted font-medium">Click camera icon to upload profile picture.</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-black text-ink-muted uppercase mb-1">Full Name</label>
                    <input type="text" value={profile.name} onChange={(e) => setProfile({...profile, name: e.target.value})} required className="w-full bg-white border border-ink/5 rounded-2xl p-3.5 text-xs font-bold text-ink focus:outline-none focus:ring-2 focus:ring-secondary/20" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-ink-muted uppercase mb-1">Phone Number</label>
                    <input type="text" value={profile.phone} onChange={(e) => setProfile({...profile, phone: e.target.value})} required className="w-full bg-white border border-ink/5 rounded-2xl p-3.5 text-xs font-bold text-ink focus:outline-none focus:ring-2 focus:ring-secondary/20" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-ink-muted uppercase mb-1">Guardian Address / Location</label>
                    <input type="text" value={profile.address} onChange={(e) => setProfile({...profile, address: e.target.value})} required placeholder="e.g. Mirpur 2, Dhaka" className="w-full bg-white border border-ink/5 rounded-2xl p-3.5 text-xs font-bold text-ink focus:outline-none focus:ring-2 focus:ring-secondary/20" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-ink-muted uppercase mb-1">Child Class / Grade Details</label>
                    <input type="text" value={profile.childClass} onChange={(e) => setProfile({...profile, childClass: e.target.value})} required placeholder="e.g. Class 10 (Science)" className="w-full bg-white border border-ink/5 rounded-2xl p-3.5 text-xs font-bold text-ink focus:outline-none focus:ring-2 focus:ring-secondary/20" />
                  </div>
                </div>

                <div className="flex justify-end">
                  <button type="submit" disabled={isSaving} className="bg-secondary text-white px-8 py-3.5 rounded-2xl font-black text-xs uppercase shadow-lg shadow-secondary/20 hover:bg-secondary-dark transition-all flex items-center gap-2 cursor-pointer">
          ...
          <div className="space-y-8">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/10 border border-white/20 rounded-2xl py-4 pl-12 pr-4 text-sm font-bold placeholder:text-white/40 focus:outline-none focus:bg-white/20 transition-all text-white"
                  />
                </div>
                <Link to="/tutors" className="w-full bg-white text-secondary py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-ink hover:text-white transition-all active:scale-95 flex items-center justify-center">
                  Search Tutors
                </Link>
              </div>
            </div>
            ...
            <div className="bg-white/60 backdrop-blur-xl p-8 rounded-[32px] border border-white/40 shadow-xl shadow-ink/5 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Link to="/guardian/saved" className="flex flex-col items-center gap-3 p-4 bg-ink/5 rounded-2xl hover:bg-secondary hover:text-white transition-all group">
                ...
                <Link to="/guardian/requests" className="flex flex-col items-center gap-3 p-4 bg-ink/5 rounded-2xl hover:bg-secondary hover:text-white transition-all group">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-secondary shadow-sm group-hover:scale-110 transition-transform">
                    <Bell size={20} />
                  </div>
                  <span className="text-[10px] font-black uppercase tracking-widest">Alerts</span>
                </Link>
                <Link to="/guardian/messages" className="flex flex-col items-center gap-3 p-4 bg-ink/5 rounded-2xl hover:bg-secondary hover:text-white transition-all group">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-secondary shadow-sm group-hover:scale-110 transition-transform">
                    <MessageSquare size={20} />
                  </div>
                  <span className="text-[10px] font-black uppercase tracking-widest">Chat</span>
                </Link>
                <Link to="/guardian/profile" className="flex flex-col items-center gap-3 p-4 bg-ink/5 rounded-2xl hover:bg-secondary hover:text-white transition-all group">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-secondary shadow-sm group-hover:scale-110 transition-transform">
                    <Settings size={20} />
                  </div>
